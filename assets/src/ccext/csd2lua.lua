...
function _M:onProperty_Node(obj, name, value)
	...
	elseif name == "OutlineSize" or name == "ShadowOffsetX" or name == "ShadowOffsetY" or name == "ShadowBlurRadius" then
		opts[name] = tonumber(value) or 0
	elseif name == "ActionTag" then
		opts[name] = tonumber(value)
	else
		return false
	end
...
function _M:handleOpts_Node(obj)
	...
	for name, value in pairs(opts) do
		...
		elseif name == "ActionTag" then
			if self.actTag ~= nil then
				for key, Object in pairs(self.actTag) do
					if(Object == value) then
						local parent_node_name = key
						table.insert(self.tblAni, string.format("	%s:setNode(roots.root.%s)\n",parent_node_name,tostring(opts.Name)))
					end
				end
			end
		end
	...
	self:writef("	roots.root.%s = obj\n",tostring(tblTmp.Name))
...
function _M:handleOpts_EditBox(obj)
	...
	if opts.Color then
		self:writef("	obj:setFontColor(%s)\n", formatColor(opts.Color))
	end

	if opts.FontName then
		self:writef("	obj:FontName(\"%s\")\n", tostring(opts.FontName))
		self:writef("	obj:PlaceHolderFontName(\"%s\")\n", tostring(opts.FontName))
	end
	...
	if opts.PasswordEnabled then
		self:write("	obj:setInputFlag(cc.EDITBOX_INPUT_FLAG_PASSWORD)\n")
	end

	self:write("	obj:setReturnType(cc.KEYBOARD_RETURNTYPE_DONE)\n")
	self:write("	obj:setInputMode(cc.EDITBOX_INPUT_MODE_SINGLELINE)\n")
end
	...
	for _, p in pairs(root:properties()) do
		...
		if not onProperty(self, obj, name, value) then
			if name ~= "IconVisible" and name ~= "ctype" and name ~= "ActionTag" and name ~= "ColorAngle" and name ~= "CanEdit" and
					name ~= "LeftEage" and name ~= "TopEage" and name ~= "RightEage" and name ~= "BottomEage" and name ~="ObjectIndex" then
...
function _M:objScriptOf(className, root)
	...
	elseif className == "GameNode" or className == "SingleNode" or className == "Node" then
		if root["@Name"]=="ListItem" then 
			obj = ccui.Widget:create()
			script = "	obj = ccui.Widget:create()\n"
		else
			obj = cc.Node:create()
	...
	elseif className == "UrlButton" then
		className = "Button"
		obj = require("ccext.UrlButton"):create()
		script = "	obj = require(\"ccext.UrlButton\"):create()\n"
	elseif className == "UrlImgView" or className == "UrlImageView" then
		className = "ImageView"
		obj = require("ccext.UrlImgView"):create()
		script = "	obj = require(\"ccext.UrlImgView\"):create()\n"
	elseif className == "EditBox" then
		...
		script = "	obj = ccui.EditBox:create(" .. formatSize(siz) .. ", cc.Scale9Sprite:create())\n"
...
function _M:csd2lua(csdFile, luaFile)
	...
	while node do
		...
		if name == "Animation" then
			...
			--TODO Timeline
			self:createAnimationTree(node,"ActionTimeline",tblAni)
		elseif name == "ObjectData" then
			self:createNodeTree(node, "NodeObjectData")
		elseif name == "AnimationList" then
			-- TODO 
			self:createAnimationListTree(node, "AnimationList",tblAni)
		end
	...
	if #tblAni > 0 then
		table.insert(tblAni, "	roots.root.animations = animations\n\n")
...
function _M:createAnimationTree(root, classType,tblAni)
	return _createAnimationTree(self, root, classType,tblAni)
end

function _M:createAnimationListTree(root, classType,tblAni)
	return _createAnimationListTree(self, root, classType,tblAni)
end

function createTimeline(self,root,rootName,parentName,index,className,tblAni)
	local parent_node_name = parentName .. index
	table.insert(tblAni, "	obj = ccs.Timeline:create()\n")
	table.insert(tblAni, "	" .. rootName .. ":addTimeline(obj)\n")
	local nodeName = ""
	local ActionTag = tonumber(root["@ActionTag"])
	table.insert(tblAni, "	" .. parent_node_name .. " = obj\n")		
	self.actTag[parent_node_name] = ActionTag
	table.insert(tblAni,"\n")
	
	local nextSiblingNode = nextSiblingIter(root)
	local node, udata = nextSiblingNode()
	while(node) do
		table.insert(tblAni, "	obj = ccs." .. className .. ":create()\n")
		table.insert(tblAni, "	" .. parent_node_name .. ":addFrame(obj)\n")
		if(className == "PositionFrame") then
			table.insert(tblAni, string.format("	obj:setFrameIndex(%d)\n", tonumber(node["@FrameIndex"]) or 0))
			table.insert(tblAni, string.format("	obj:setTween(true)\n"))
			table.insert(tblAni, string.format("	obj:setX(%s)\n", tostring(node["@X"]) or 1))
			table.insert(tblAni, string.format("	obj:setY(%s)\n", tostring(node["@Y"]) or 1))
		elseif(className == "ScaleFrame") then
			table.insert(tblAni, string.format("	obj:setFrameIndex(%d)\n", tonumber(node["@FrameIndex"]) or 0))
			table.insert(tblAni, string.format("	obj:setTween(true)\n"))
			table.insert(tblAni, string.format("	obj:setScaleX(%s)\n", tostring(node["@X"]) or 1))
			table.insert(tblAni, string.format("	obj:setScaleY(%s)\n", tostring(node["@Y"]) or 1))
		elseif(className == "RotationSkewFrame") then
			table.insert(tblAni, string.format("	obj:setFrameIndex(%d)\n", tonumber(node["@FrameIndex"]) or 0))
			table.insert(tblAni, string.format("	obj:setTween(true)\n"))
			table.insert(tblAni, string.format("	obj:setSkewX(%s)\n", tostring(node["@X"]) or 1))
			table.insert(tblAni, string.format("	obj:setSkewY(%s)\n", tostring(node["@Y"]) or 1))
		end
		table.insert(tblAni,"\n")
		node  = nextSiblingNode()
	end
...
function _createAnimationTree(self, root, classType,tblAni)
	self.actTag = {}
	local t_name = "animations."
	local rootName = t_name .. "actionTimeline"
	local parentName = t_name .. "timeline"
	table.insert(tblAni, "	animations = {}\n")
	table.insert(tblAni, "	obj = ccs.ActionTimeline:create()\n")
	table.insert(tblAni, "	" .. rootName .. " = obj \n")
	table.insert(tblAni, string.format("	obj:setDuration(%d)\n", tonumber(root["@Duration"]) or 0))
	table.insert(tblAni, string.format("	obj:setTimeSpeed(%s)\n", tostring(root["@Speed"]) or 1))
	table.insert(tblAni,"\n")
	...
	if root then
		...
		while node do
			_j = _j + 1
			if node["@Property"] == "Position" then
				className1 = "PositionFrame"
			elseif node["@Property"] == "Scale" then
				className1 = "ScaleFrame"
			elseif node["@Property"] == "RotationSkew" then
				className1 = "RotationSkewFrame"
			end
			createTimeline(self,node,rootName,parentName,_j,className1,tblAni)
			node = nextSiblingNode()
		end
	else
		self:write("\n")
...
function _createAnimationListTree(self, root, classType,tblAni)
	local t_name = "animations."
	local rootName = t_name .. "actionTimeline"
	local tblTmp = {}
	if root then
		...
		while node do
			...
			for _, p in pairs(node:properties()) do
				local name = p.name
				local value = p.value
				if name == "Name" then
					tblTmp[name] = value
				elseif name == "StartIndex" then
					tblTmp[name] = value
				elseif name == "EndIndex" then
					tblTmp[name] = value
				end
			end
			
			table.insert(tblAni, string.format("	obj = {name=\"%s\", startIndex=%d, endIndex=%d}\n",tostring(node["@Name"]),tonumber(node["@StartIndex"]),tonumber(node["@EndIndex"])))
			local parentName = t_name .. tostring(node["@Name"])
			table.insert(tblAni, string.format("	%s = obj\n", parentName))
			table.insert(tblAni, string.format("	%s:addAnimationInfo(obj)\n", rootName))
			table.insert(tblAni,"\n")
